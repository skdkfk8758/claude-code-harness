...
## 개요
CCH는 Claude Code에서 동작하는 경량 오케스트레이션 플러그인이다. Tier 기반 스킬 강화, Beads 태스크 추적, Hook 자동화를 제공하며, 설계→구현→검증 파이프라인을 통합한다.
...
| cch-commit | Analyze changes and create logical, well-structured commits with Bead trailers. |
| cch-todo | Show all tasks from Beads (SSOT) and current session TaskList. |
| cch-verify | Verify implementation before claiming completion. Runs tests, checks output, validates against spec. Absorbs debug and TDD workflows. |
...
| cch-pr | Create a pull request with Beads linking, TODO references, and structured description. |
...
# 개별 테스트
bash tests/harness.sh test_contract
bash tests/harness.sh test_skill
bash tests/harness.sh test_beads
bash tests/harness.sh test_workflow
bash tests/harness.sh test_resilience
```
