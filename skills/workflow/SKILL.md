...
After each Skill step (all gate levels):
1. Record the gate level as `gateLevel`
2. For `approval`: record the user's response as `userFeedback`
3. For `checkpoint`: record `userFeedback: "auto-proceed"` or user's intervention comment
4. For `auto`: record `userFeedback: null`
5. Perform knowledge indexing if output file exists (see Knowledge Graph Management > Indexing)
...
Skill steps are automatically dispatched by the orchestrator. The `gate` field controls
...
| Level | 동작 | YAML 값 |
|-------|------|---------|
| **approval** | 자동 실행 → 결과 표시 → 사용자 명시적 승인 대기 | `gate: approval` |
| **checkpoint** | 자동 실행 → 요약 표시 → 사용자 개입 없으면 자동 진행 | `gate: checkpoint` |
| **auto** | 자동 실행 → 로그만 출력 → 자동 진행 | `gate: auto` |

하위 호환: `gate: user-approval` → `gate: approval`로 자동 매핑.
`gate` 필드 없는 skill step → `gate: approval` 기본값 (안전 우선).
...
1. Announce the step:
   ```
   ────────────────────────────────────
   [Auto-Skill] Step {N}/{total}: {id} ({skill-name})
   {description}
   Gate: {gate-level}
   ────────────────────────────────────
   ```
2. Resolve the skill prompt:
   - Use `resolve-skill({skill-name})` (see Resource Resolution)
   - Read the skill's SKILL.md content
3. Build the dispatch prompt:
   - Include the skill's full SKILL.md content as the agent's instructions
   - Append context: previous step outputs (read the files)
   - If step has `input` field, include the input file content
   - Append knowledge context (see Knowledge Graph Management > Context Injection)
   - Add instruction header:
     ```
     ## Workflow Context
     You are being auto-dispatched as part of workflow "{workflow-name}", step {N}/{total}.
     Execute the skill instructions below. Use AskUserQuestion when user input is needed
     (e.g., design interviews, option selection).
     ```
4. Dispatch via **Agent tool** with `subagent_type: "general-purpose"`
5. Apply gate-level post-processing:

   **`gate: approval`**:
   - Present the agent's results to the user
   - Display:
     ```
     [Auto-Skill] {id} 완료. 결과를 확인해주세요.
     승인하시겠습니까? (승인 / 수정 요청 / 재실행)
     ```
   - Wait for explicit user response:
     - 승인 패턴 (`ㅇㅇ`, `응`, `승인`, `ㄱㄱ`, `yes`, `approve`) → proceed
     - 수정 요청 → re-dispatch agent with user feedback
     - 재실행 → re-dispatch from scratch
   - Max 3 re-dispatches, then escalate

   **`gate: checkpoint`**:
   - Display concise summary (2-3 lines):
     ```
     [Checkpoint] {id} 완료: {auto-extracted summary}
     산출물: {output path}
     개입하려면 지금 말씀해주세요. 아니면 다음 스텝으로 진행합니다.
     ```
   - Use AskUserQuestion with a timeout hint:
     - If user responds with intervention → handle feedback, re-dispatch if needed
     - If user responds with proceed signal (`ㅇㅇ`, `넘어가`, `skip`, `next`, Enter) → proceed
   - Auto-proceed to next step

   **`gate: auto`**:
   - Log completion only:
     ```
     [Auto-Skill] {id} ✓ 완료 — {one-line summary}
     ```
   - Proceed immediately to next step

6. Verify output file exists (if `output` field defined), update state.
7. Record gate result in state:
   ```json
   {
     "status": "completed",
     "gateLevel": "approval",
...
|-----------|--------------|
| `type: skill` | 스킬 실행 완료 후 (gate level 무관), output 파일이 존재하면 인덱싱 |
...
|-----------|------------------|
| `type: skill` | 적용 — 오케스트레이터가 auto-dispatch하므로 dispatch prompt에 knowledge context 추가 |
...
- Never skip a `gate: approval` step — approval gates exist to enforce user decisions
- `gate: checkpoint` and `gate: auto` steps may proceed without explicit approval
