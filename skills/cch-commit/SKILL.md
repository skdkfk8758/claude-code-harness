---
name: cch-commit
description: Analyze changes and create logical, well-structured commits with Plan trailers.
user-invocable: true
...
변경사항을 분석하여 논리적 단위로 그룹화하고, Plan 트레일러가 포함된 커밋을 생성합니다.
...
5. `.claude/cch/execution-plan.json` 읽기 (work_id 확인)
6. `git rev-parse --abbrev-ref HEAD` (현재 브랜치명)

Step 1 완료 후 **플랜 문서를 결정**합니다 (순서대로 시도):

1. `.claude/cch/branches/<branch>.yaml` 파일이 있으면 읽어 `work_id` 필드 추출
2. 없으면 `.claude/cch/execution-plan.json`에서 `work_id` 추출
3. 둘 다 없으면 플랜 없음으로 진행

결정된 플랜 정보는 Step 4 커밋 트레일러 및 Step 7 보고에 사용됩니다.
...
Branch: <현재 브랜치명> (plan: <연결된 work_id 또는 "없음">)
