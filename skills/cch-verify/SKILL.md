...
- **Tier 1+**: `superpowers:verification-before-completion` 워크플로우를 인라인으로 적용
  - 증거 기반 검증 체크리스트 자동 생성
  - 검증 명령 실행 및 출력 캡처
- **Tier 1+**: `superpowers:systematic-debugging` 을 Step 3에서 활용
  - 구조화된 가설 수립 및 검증 프로세스
  - Binary search 기법으로 원인 범위 좁히기
- **Tier 1+**: `superpowers:test-driven-development` TDD 사이클 지원
