...
세션 시작
  └─ checkEnv():
       ├─ plugins 설치 여부 체크 (omc? superpowers? 기타?)
       ├─ MCP 서버 목록 확인
...
cch-verify:     verify-lite (Tier 0)     ↔ omc verifier agent (Tier 1)
cch-review:     review-lite (Tier 0)     ↔ omc code-reviewer (Tier 1)
cch-team:       순차 파이프라인 (Tier 0) ↔ omc 병렬 에이전트 (Tier 1)
```
...
omc가 없으면 Agent 도구 없이 직접 수행하고, 있으면 Agent 도구를 활용한다.
...
## 강화 (사용 가능할 때)
- omc Agent 사용 가능시: analyst + architect 에이전트를 병렬로 활용
...
"설치 가이드"가 슬래시 커맨드(`/cch-guide-omc`)일 필요가 있는가?
...
docs/
└── guides/
    ├── omc.md             # omc 설치/설정/활용 가이드
    ├── superpowers.md     # superpowers 가이드
    └── extensions.md      # gptaku, ruflo, excalidraw 등
```
...
- 사용자가 `/cch-adapter-omc`를 직접 실행할 일이 없다
